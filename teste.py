...
# Inicialização do encoder e buffer
encoder = Encoder()
buffer = io.BytesIO()
...
while buffer.tell() < len(buffer.getvalue()):
    # Supondo que cada quadro é um array com 800x600 de resolução e canal extra (4 bytes por pixel para XBGR8888)
    frame_data = buffer.read(800 * 600 * 4)  # Lê cada quadro com os 4 canais (XBGR)
    if not frame_data:
        break
    ...
    frame_bgr = frame[:, :, 1:]  # Remove o canal alfa 'X'
